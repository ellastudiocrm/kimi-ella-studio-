...
export async function POST(request: Request) {
  ...
  // Valida perfil admin/gestor via RLS (consulta a usuarios_internos)
  ...
  try {
    ...
    if (!file || !servicoId) {
      return NextResponse.json({ error: 'Ficheiro e ID do serviço obrigatórios' }, { status: 400 })
    }
    ...
    const path = `${usuario.empresa_id}/${servicoId}/${Date.now()}.${ext}`
    ...
    const {
      data: { publicUrl },
    } = admin.storage.from('servicos').getPublicUrl(path)
