...
export default function ProfissionaisPage() {
  ...
  const cardapio = window.location.pathname.startsWith('/ella_men') ? 'ella_men' : 'ella_studio'
  ...
  return (
    <div className="min-h-screen px-4 py-6">
      ...
      <p className="text-gray-600 mb-6">
        Profissionais disponíveis para <span className="font-medium">{servicoNome}</span>:
      </p>
      ...
      <div className="space-y-4">
        {profissionais.map((prof) => (
          <Link
            key={prof.id}
            href={`/agendar?profissional=${prof.id}&servico=${servicoSlug}&cardapio=${cardapio}`}
