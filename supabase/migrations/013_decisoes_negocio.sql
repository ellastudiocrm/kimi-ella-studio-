-- 013_decisoes_negocio.sql (v3.4.3) — 3 decisões de negócio fechadas pelo dono (27/07/2026)
--
-- 1. CARDÁPIO OBRIGATÓRIO: remove o fallback 'feminino'. O campo cardapio é exigido
--    em cada item de p_itens — a Ella (atendente) e o frontend futuro perguntam sempre.
--    Fonte: docs/DECISOES.md, Decisão 1.
--
-- 2. SINAL 30% FIXO (fase inicial): política de sinal para todos os serviços que
--    exijam pagamento. O default da tabela reservas já é 30, mas esta migration
--    deixa o comentário de registo. Rever após 3 meses de operação.
--    Fonte: docs/DECISOES.md, Decisão 2.
--
-- 3. AVALIAÇÃO GRATUITA (preço 0): quando v_total = 0, a reserva confirma diretamente
--    sem PIX. A cobrança de valor 0 é criada como REGISTO INTERNO (recibo/rasto
--    para relatórios futuros) — Alternativa A. A alternativa B (não criar cobrança)
--    foi rejeitada por exigir refatorar registrar_pagamento_presencial e
--    processar_pagamento_webhook (risco de regressão em funções de dinheiro).
--    Fonte: docs/DECISOES.md, Decisão 3.
--
-- Regra permanente: migrations aplicadas são imutáveis. Esta migration é nova.
...
BEGIN
    ...
    -- POLÍTICA DE SINAL: 30% fixo (fase inicial). Rever após 3 meses de operação.
    ...
    LOOP
        ...
        -- Decisão 1: cardápio OBRIGATÓRIO. Nenhum fallback.
        v_cardapio := v_item->>'cardapio';
        IF v_cardapio IS NULL OR v_cardapio = '' THEN
            RAISE EXCEPTION 'Indica o cardápio: ella_studio ou ella_men';
        END IF;
    ...
    -- Decisão 3: cobrança de valor 0 é registo interno (Alternativa A).
    -- O PIX não é gerado porque v_sinal <= 0 confirma a reserva imediatamente.
    ...
    -- v3.4.1 (auditoria #3): sinal ZERO (percentual_sinal = 0) ou avaliação gratuita
    -- (valor_total = 0) confirma diretamente, sem PIX, sem expires_at.
    IF v_sinal <= 0 THEN
...
-- REVOKE/GRANT: preservar permissões já concedidas (a função existe, CREATE OR REPLACE mantém o dono)
